...
const BlackCard = ({ balance }:{balance: number}) => (
  <div className="col-span-12 lg:col-span-5 bg-themeBlack w-full themeRadius px-4 md:px-10 pt-10 pb-6">
    <p className="text-sm text-white">YOUR WALLET BALANCE</p>
    <h2 className="text-lg lg:text-3xl xl:text-4xl text-themeGreen">{`N${numeral(balance).format('0,0.00')}`}</h2>
    <div className="flex justify-between flex-nowrap mt-6">
      <button type="button" aria-label="Ref" className="p-2 themeRadius text-themeBlue bg-themeBlueAlt text-sm  mr-5 w-full justify-center items-center focus:outline-none border-4 border-themeBlueAlt hover:border-themeBlue flex flex-nowrap">
      ...
      <button type="button" aria-label="Fund Wallet" className="p-2 themeRadius text-themeGreen border border-themeGreen text-sm w-full justify-center items-center focus:outline-none hover:bg-themeGreen hover:text-white">
        Fund Your Wallet
      </button>
    </div>
  </div>
